...
public class A_EntityMangerLifeCycleTests {
    /*
    * 엔티티 매니저 팩토리(EntityMangerFactory)란?
    * 앤티티 매니저를 생성할 수 있는 기능을 제공하는 팩토리 클래스이다.
    * Thread-safe하기 떄문에 여러 스레드가 동시에 접근해도 안전하므로 서로 다른 스레드 간 공유해서 재사용한다.
    * Thread-safe란 기능을 요청 스코프마다 생성하기에는 비용(시간,메모리) 부담이 크므로
    * Application 스코프와 동일하게 싱글톤으로 생성해서 관리하는 것이 효율적이다.
    * 따라서 데이터베이스를 사용하는 애플리케이션 당 한 개의 EntityManagerFactory를 생성한다.
    * */

    /*
    * 엔티티 매니저(Entity Manager)란? 데이터베이스에 명령을 내리는 인터페이스
    ...
    * 영속성 컨텍스트는 엔티티 매니저를 생성할 떄 하나 만들어진다.
    ...
    * */
    ...
    public void initManger(){
        ...
        // 영속성 컨텍스트에 접근 가능한것 생성
    }

    @Test
    public void 엔터티_매니저_팩토리와_엔터티_매니저_생명주기_확인1(){
        System.out.println("entityMangerFactory.hascode : " + entityManagerFactory.hashCode());
        System.out.println("entityManger.hascode : " + entityManager.hashCode());
    }
