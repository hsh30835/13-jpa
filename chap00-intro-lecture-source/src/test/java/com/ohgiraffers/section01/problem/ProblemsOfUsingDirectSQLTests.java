...
class ProblemsOfUsingDirectSQLTests {
    ...
    void closeConnection() throws SQLException {
        con.rollback(); //테스트코드도 db에 남기떄문에 테스트가 끝나면 롤백시켜준다
        con.close();
    }
    ...
    * 2. SQL에 의존하여 개발
    * 3. 패러다임의 불일치 (상속, 연관관계, 객체 그래프 탐색)
    * 4. 동일성 보장 문제
    * */

    // 1. 데이터 변환, SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하)
    @DisplayName("직접 SQL을 작성하여 메뉴를 조회할 때 발생하는 문제확인")
    @Test
    ...
    @DisplayName("직접 SQL을 작성하여 신규 메뉴를 추가할때 발생하는 문제 확인")
    @Test
    ...
    /*
    * 2. SQL에 의존하여 개발
    * 요구사항의 변경에 따라 어플리케이션의 수정이 SQL의 수정으로도 이어진다.
    * 이러한 수정 영향을 미치는 것은 오류를 발생시킬 가능성도 있지만 유지보수성에서도 악영향을 미친다.
    ...
    * 3-1 상속 문제
    * 객체 지향 언어의 상속 개념과 유사한 것이 데이터 베이스의 서브타입앤티티이다.
    * 유사한 것 같지만 다른 부분은 데이터베이스의 상속 개념을 데이터로 추상화해서 슈퍼타입과 서브타입으로 구분하고,
    ...
    * 물리적으로 회원과 법인회원 테이블은 분리된 상황에서는 각각 insert 구문을 수행해야 한다.
    * insert into 회원
    * insert into 법인회원
    * 하지만 jpa를 이용하여 상속을 구현하는 경우에는
    * entityManager.persist(법인회원)
    * 이렇게 한 번에 해결하는 것이 가능하다.
    * */

    /*
    * 3-2. 연관관계 문제, 객체 그래프 탐색 문제
    * 객체지향에서 말하는 가지고 있는(associtation 연관관계 혹은 Collection 연관관계) 경우 데이터베이스 저장 구조와 다른형태이다.
    ...
    *
    ...
    // 4. 동일성 보장 문제
    @DisplayName("조회한 두 개의 행을 담은 객체를 동일성 비교 테스트")
    @Test
