...
public class ProjectionTests {
    ...
    * 1. 엔티티 프로젝션
    *    원하는 객체를 바로 조회할 수 있다.
    *    조회할 엔티티는 연속성 컨텍스트가 관리한다.
    *
    * 2. 임베디드 타입 프로젝션(임베디드 타입에 대한 설명은 MENUInfo 클래스에서 설명)
    *    엔티티와 거의 비슷하게 사용되며 조회의 시작점이 될 수 없다. -< From 절에서 사용불가
...
//        Assertions.assertNotNull(categoryOfMenu.getMenuList());
    ...
    /* 3. 스칼라 타입 프로잭션 */
    @Test
    void TypeQuery를_이용한_스칼라_타입_프로잭션_테스트(){
        // when
    ...
    /*
     * 조회하려는 컬럼 값이 1개인 경우 TypeQuery로 반환 타입을 단일 타입 값에 대해 지정할 수 있지만 다중 열 컬럼을 조회하는 경우 타입을 지정하지 못한다.
     * 그때 TypeQuery 대신 Query를 사용하여 Object[]로 행의 정보를 반환 받아 사용한다.
     * */
    @Test
    void Query를_이용한_스텔라_타입_프로잭션_테스트(){
