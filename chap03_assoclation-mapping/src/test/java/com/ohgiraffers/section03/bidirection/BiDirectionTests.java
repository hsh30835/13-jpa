...
public class BiDirectionTests {
    ...
    * 비즈니스 중요도를 배제하고 단순히 외래키 관리자의 어미를 부여해야 한다.
    * 연관 관계의 주인은 외래키를 가지고 있는 엔터티이다.
    *
    * 메뉴 엔터티는 카테고리 엔터티와 카테고리 코드를 참조하고 있다.
    * 즉 외래키를 가지고 있는 메뉴 엔터티가 연관관계의 주인이다. (핵심)
    * */

    @Test
    void 양방향_연관관계_매핑_조희_테스트(){
        int menuCode = 10;
        int categoryCode = 10;

        // 진짜 연관 관계는 처음 조회 부터 조인한 결과를 인출해온다.
        ...
        // category에 포함 된 메뉴 목록 출력 구문을 작성하고 나면 실제 사용에 필요해지기 떄문에 가짜 연관 관계에 해당하는
        // 엔티티도 다시 조회하는 쿼리가 한번더 동작한다.
        foundCategory.getMenuList().forEach(System.out::println);
    }

    /*
    * 연관관계의 주인에는 값을 입력하고, 주인이 아닌 곳에는 값을 입력하지 않았을 때
