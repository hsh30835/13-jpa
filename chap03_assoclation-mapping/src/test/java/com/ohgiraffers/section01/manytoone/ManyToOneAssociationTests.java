...
public class ManyToOneAssociationTests {
    ...
    * - 1:N(OneToMany) 연관관계
    * - 1:1(OneToOne) 연관관계
    * - N:N(ManyToMany) 연관관계 // 사실상 고려 안함 거의 안씀 서로가 서로를 참조하는데 이러면 데이터가 중복됨
    *
    * 방향에 따른 분류
    ...
    * 객체 지향 모델에 맞게 작성된 쿼리를 통해, 엔티티 객체를 대상으로 검색, 검색 결과를 토대로 객체를 조작할 수 있다.
    * join 문법이 sql과 다소 차이가 있지만 직접 쿼리를 작성할 수 있는 문법을 제공한다.
    * 주의할 점은 FROM절에 기술할 테이블 명에는 반드시 엔티티명이 작성되어야 한다.
    * */
    @Test
    void 디테일_연관관계_객체지향쿼리_사용한_카테고리_이름_조회_테스트(){
        //given
    ...
    /* Commit()을 할 경우 컨텍스트 내에 저장된 영속성 객체를 insert하는 쿼리가 동작된다.
    ...
    * cascade=Cascade.PERSIST를 설정하면 MenuAndCategory 엔티티를 영속화 할 때 Category 엔티티도 함께 영속화 된다.
    * */
    @Test
    ...
    void 영속성_삭제_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,100006);
        ...
        transaction.commit();
        MenuAndCategory deletMenu = entityManager.find(MenuAndCategory.class,100006);
    ...
    void Merge_update_테스트(){
        ...
        Category category = entityManager.find(Category.class,33336);
    ...
    void detach_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class, 100005);
        menuAndCategory.setMenuName("변경함");
        Category category = menuAndCategory.getCategory();
        category.setCategoryName("detach 카테고리 수정");
        menuAndCategory.setCategory(category);
